...
import org.apache.griffin.measure.step.transform.{CustomOperatorTransformStep}

case class CustomOperatorDQStepBuilder() extends RuleParamStepBuilder {

  def buildSteps(context: DQContext, ruleParam: RuleParam): Seq[DQStep] = {
    val name = getStepName(ruleParam.getOutDfName())
    val transformStep = CustomOperatorTransformStep(
